  ...
  async fetch(request, env, ctx) {
    ...
    if (pathname.startsWith('/api/ws/')) {
      ...
      server.addEventListener('message', (event) => {
        try {
          handleWebSocketMessage(event.data, server, role, room);
        } catch (error) {
  ...
  try {
    ...
    if (typeof data === 'string') {
      ...
      switch (parsedData.type) {
        ...
        case 'file-metadata':
          room.fileMetadata = parsedData.metadata;
            ...
            receiverSocket.onmessage = function(event) {
                try {
                    ...
                    if (event.data instanceof ArrayBuffer) {
                        ...
                        // 更新进度
                        const received = receivedChunks.reduce(function(a, b) { return a + b.byteLength; }, 0);
